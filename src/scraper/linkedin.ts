...
export async function scrapeLinkedInPost(
  ...
  try {
    ...
    if (statusCode === 404) {
      throw new ScraperException(
        "POST_NOT_FOUND",
        "The requested LinkedIn post was not found (404).",
      );
    }
    if (statusCode === 999 || statusCode === 403) {
      throw new ScraperException(
        "LINKEDIN_BLOCKED",
        "LinkedIn blocked the request (Anti-bot detection triggered).",
      );
    }
    ...
    if (
      ...
      throw new ScraperException(
        "LOGIN_REQUIRED",
        "LinkedIn redirected to a login wall. Credentials are required to view this post.",
      );
    }
    ...
    if (
      ...
      throw new ScraperException(
        "CAPTCHA_REQUIRED",
        "LinkedIn triggered a security verification CAPTCHA.",
      );
    }
    ...
    if (
      (await page
        .locator(".login-form, #login-submit, #username, #password")
        .count()) > 0
    ) {
      throw new ScraperException(
        "LOGIN_REQUIRED",
        "LinkedIn login form detected on the page.",
      );
    }
    ...
    if (
      ...
      throw new ScraperException(
        "POST_NOT_FOUND",
        "This post is not found or is no longer available to public viewers.",
      );
    }
    ...
    try {
      ...
      console.warn(
        "Post container selector not found, proceeding to parse via fallback selectors.",
      );
    }
    ...
    if (errMsg.includes("timeout") || errMsg.includes("Timeout")) {
      throw new ScraperException(
        "SCRAPE_TIMEOUT",
        "The scraping process timed out while loading the page.",
      );
    }
    throw new ScraperException(
      "INTERNAL_ERROR",
      `An unexpected error occurred during scraping: ${errMsg}`,
    );
  } finally {
